...
class DataModule {
    ...
    fun provideClientDao(database: AppDatabase): ClientDao {
        return database.clientDao() // Assuming you meant to provide the same Dao for recharges
    }
    ...
    @Single
    fun provideClientRepository(clientDao: ClientDao): ClientRepositoryImpl {
        return ClientRepositoryImpl(clientDao) // Assuming you meant to provide the same repository for products
    }
}